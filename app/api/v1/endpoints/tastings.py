...
async def list_tasting_sessions(
    ...
    try:
        if coffee_id:
            tastings = await tasting_repository.get_by_coffee_id(
                db, coffee_id=coffee_id, user_id=current_user_id, skip=skip, limit=limit
            )
            ...
            tastings = await tasting_repository.get_by_user_id(db, current_user_id, skip=skip, limit=limit)
        ...
        logger.error("Error listing tasting sessions", user_id=current_user_id, error=str(e))
...
async def create_tasting_session(
    ...
    try:
        ...
        if not coffee:
            raise HTTPException(
                status_code=404, detail=f"Coffee with ID '{tasting_data.coffee_id}' not found"
            )

        # Create the tasting session with notes
        tasting = await tasting_repository.create_with_notes(
            db, tasting_data=tasting_data, user_id=current_user_id
        )
        ...
        logger.error("Error creating tasting session", user_id=current_user_id, error=str(e))
        raise HTTPException(status_code=500, detail="Internal server error") from e
...
async def get_tasting_session(
    ...
    try:
        ...
        if tasting.user_id != current_user_id:
            raise HTTPException(
                status_code=403, detail="You don't have permission to access this tasting session"
            )
        ...
        logger.error(
            "Error getting tasting session", session_id=str(session_id), user_id=current_user_id, error=str(e)
        )
        raise HTTPException(status_code=500, detail="Internal server error") from e
...
async def update_tasting_session(
    ...
    try:
        ...
        if tasting.user_id != current_user_id:
            raise HTTPException(
                status_code=403, detail="You don't have permission to update this tasting session"
            )
        ...
        if update_data.coffee_id:
            ...
            if not coffee:
                raise HTTPException(
                    status_code=404, detail=f"Coffee with ID '{update_data.coffee_id}' not found"
                )

        # Update the tasting
        await tasting_repository.update(db, db_obj=tasting, obj_in=update_data)
