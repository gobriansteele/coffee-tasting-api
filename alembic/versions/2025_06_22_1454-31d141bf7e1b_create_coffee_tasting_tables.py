...
def upgrade() -> None:
    ...
    sa.Column('processing_method', postgresql.ENUM('WASHED', 'NATURAL', 'HONEY', 'SEMI_WASHED', 'WET_HULLED', 'CARBONIC_MACERATION', 'OTHER', name='processing_method_enum'), nullable=True),
    sa.Column('variety', sa.String(length=255), nullable=True),
    sa.Column('roast_level', postgresql.ENUM('LIGHT', 'MEDIUM_LIGHT', 'MEDIUM', 'MEDIUM_DARK', 'DARK', name='roast_level_enum'), nullable=True),
    ...
    op.create_table('coffee_flavors',
    ...
    sa.ForeignKeyConstraint(['flavor_tag_id'], ['flavortag.id'], ),
    sa.PrimaryKeyConstraint('coffee_id', 'flavor_tag_id')
    )
    op.create_table('tasting_session',
    sa.Column('coffee_id', sa.UUID(), nullable=False),
    sa.Column('user_id', sa.String(length=255), nullable=False),
    sa.Column('brew_method', postgresql.ENUM('POUR_OVER', 'FRENCH_PRESS', 'ESPRESSO', 'AEROPRESS', 'CHEMEX', 'V60', 'KALITA', 'SIPHON', 'COLD_BREW', 'MOKA_POT', 'DRIP', 'OTHER', name='brew_method_enum'), nullable=False),
    sa.Column('grind_size', postgresql.ENUM('EXTRA_FINE', 'FINE', 'MEDIUM_FINE', 'MEDIUM', 'MEDIUM_COARSE', 'COARSE', 'EXTRA_COARSE', name='grind_size_enum'), nullable=True),
    ...
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['flavor_tag_id'], ['flavortag.id'], ),
    sa.ForeignKeyConstraint(['tasting_session_id'], ['tasting_session.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index(op.f('ix_tasting_note_tasting_session_id'), 'tasting_note', ['tasting_session_id'], unique=False)
